...
import PaymentListFilter from './PaymentList.filter'
import PaymentListing from './PaymentList.listing'
import PaymentListPagination from './PaymentList.pagination'
...
const initialState = {
  ...
  message: {
    ...
    delete_confirm_title: 'Delete Payment',
    delete_confirm: 'Are you sure you want to delete this payment? The amount will be added back to the member\'s due balance.',
    delete_bulk_select: 'Please select item(s)',
    delete_bulk_confirm: 'Are you sure you wish to delete selected payments?',
  },
}

const PaymentListController: FC<any> = (props) => {
  ...
  const {
    ...
    reloadView,
    reloadForm,
  } = useCrudListService(PaymentApi, queryState, initialState, props)
  ...
  return (
    <div className='card'>
      <Form form={formRef} name='paymentListingFilterForm' initialValues={initialValues}>
        <PaymentListFilter
          filters={filters}
